...
const Login = ({navigation}) => {
  ...
  return (
    <View style={styles.container}>
      ...
      />
      {state !== 0 ? <Input ref={inputPassword} label="Password" value={password} onChangeText={text => setPassword(text)} secureTextEntry={true}/> : null}
      {state === 2 ? <Input label="Confirm password" value={confirmPassword} onChangeText={text => setConfirmPassword(text)} secureTextEntry={true}/> : null}
      <Button
        title={loginTitle}
        onPress={() => checkLogin()}
        />
      {state === 2 ? <Button ref={btnBack} title="Login" onPress={() => {setState(1); inputEmail.current.clear(); inputPassword.current.clear();}} /> : null}
    </View>
  );
};
